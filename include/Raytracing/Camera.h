...
class Camera {
  public:
    Camera(int width, float aspect_ratio, float fov, Point3 position,
  ...
  private:
    ...
    int sample_count = 20;
    int render_depth = 50;
    int render_thread_count = std::thread::hardware_concurrency() == 0
                                  ? 12
                                  : std::thread::hardware_concurrency();
    int chunk_dimension =
        width / render_thread_count < 0 ? width : width / render_thread_count;
    float dof_angle = 0;
